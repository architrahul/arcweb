...
export function AtlasMap() {
  return (
    <section className="relative min-h-[calc(100vh-65px)] overflow-hidden bg-[var(--map-surface)]">
      <Image
      ...
      />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_38%,rgba(255,252,242,0.06)_0%,rgba(247,243,232,0.16)_42%,var(--home-vignette)_100%)]" />
      <div className="absolute inset-x-0 top-0 h-44 bg-gradient-to-b from-[var(--home-top-fade)] to-transparent" />
      ...
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col gap-5 px-5 py-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            {tabs.map((tab) => (
              <Link
                className="rounded border border-[var(--line)] bg-[var(--map-tab-bg)] px-3 py-2 text-[var(--ink-muted)] shadow-sm backdrop-blur-sm transition duration-300 ease-out hover:border-[var(--accent)] hover:text-[var(--foreground)]"
                href={tab.href}
                key={tab.href}
              >
                {tab.label}
